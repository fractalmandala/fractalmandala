...
import { CHAT_COMPLETION_ENDPOINT, DEFAULT_SYSTEM_MESSAGE } from '$lib/buildapp/sharedconstants';
import { user$, userSettings$ } from '$lib/buildapp/stores';
import {
  GPT_MODEL,
  MESSAGE_ROLE,
  type Chat,
...
export const chatCompletion = async (
  ...
  const completionMessage = completion?.choices?.map?.((choice:any) => ({
...
export const hasuraGraphqlRequest = async <T>(
  ...
  const body = JSON.stringify({
    query,
    variables: (variables) || (variables) ? null : variables
  });
...
export function resizeTextarea(event:any) {
