...
import { DB_NAME } from "$lib/prompta/constants";
import {
  db,
  sqlite,
  currentThread,
...
// version number.
import schema from "$lib/prompta/migrations/00_init.sql?raw"; // NOTE that this is ?raw imported, while migrations are URL imported

// @note Migrations requrie no asset inlining. See vite.config.ts
import migration_01 from "$lib/prompta/migrations/01_add_archived.sql?url";
import migration_02 from "$lib/prompta/migrations/02_add_fts.sql?url";
