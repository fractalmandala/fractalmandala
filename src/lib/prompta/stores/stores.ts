...
import { ChatMessage, DatabaseMeta, Preferences, Thread } from "$lib/prompta/db";
import { nanoid } from "nanoid";
import { initOpenAi } from "$lib/prompta/llm/openai";
import { fetchEventSource, type EventSourceMessage } from "@microsoft/fetch-event-source";
import { wdbRtc } from "@vlcn.io/sync-p2p";
import { getSystem } from "$lib/prompta/gui";
import { dev } from "$app/environment";
import { emit } from "$lib/prompta/capture";
...
export const syncStore = (() => {
  ...
  const onConnectionChanged = (pending: any, established: any) => {
  ...
  return {
    ...
    async connectTo(s: string, remainingRetries = 3): Promise<any> {
      if (rtc) {
        rtc.connectTo(s);
