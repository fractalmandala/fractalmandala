<script lang="ts">
  import type { ChatMessage } from "$lib/prompta/db";
  ...
  import "./markdown.css";
  import { inProgressMessageId } from "$lib/prompta/stores/stores";
  let _class: string = "";
...
<style>
  ...
  .has-cursor > *:last-child:not(ul):not(ol):not(blockquote)::after,
  .has-cursor > *:not(p):last-child > :last-child::after {
    @apply font-mono;
    content: "";
